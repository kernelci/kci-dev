...
kci-dev mestro validate boots  --all-checkouts --days 7
...
kci-dev validate builds --all-checkouts --days 3 --table-output

# Specific checkout (boots) for x86_64 only
kci-dev validate boots \
...
kci-dev validate builds --history \
...
kci-dev validate builds --git-folder /path/to/linux --latest
```
