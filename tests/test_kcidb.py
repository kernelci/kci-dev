...
class TestGenerateCheckoutId:
    def test_invalid_json_argument(self):
        with pytest.raises(json.JSONDecodeError):
            # Invalid JSON must not be silently accepted as checkout input.
            json.loads('{"origin": "myci", "repo_url": "https://repo"')

    def test_missing_required_argument(self):
        # Missing the required `patchset_hash` is still allowed (has default),
        # but omitting a required positional argument should raise.
        with pytest.raises(TypeError):
            generate_checkout_id("myci", "https://repo", "main")
